...
In the Queue composer, Enter with text adds that text to the queue. On an empty composer, Enter sends the Queue head when one exists; if Queue is empty, it becomes a direct terminal Enter keystroke, exactly like pressing Enter in the terminal above. **Send next** provides the same explicit Queue-head action as empty Enter.
...
Session Manager can launch a real Claude Code or Codex CLI worker directly in a project's configured working directory. Clicking a worker launch action explicitly requests an autonomous agent, so projects without a saved worker policy default to **Full access**. **Disabled** is persisted explicitly and blocks worker launch; **Ask for permissions** can be selected when a supervised worker is preferred. Workers are tagged in tmux and recorded as session notes so Lighthouse can discover them. A global maximum (default 2) prevents unbounded creation, and repeated requests reuse an existing tagged worker.

The **New Claude Code session** and **New Codex session** actions use the same launch path: after an engine is explicitly selected, the configured CLI starts in the new tmux session. Interactive sessions always use safe **Ask for permissions** mode and are isolated from the worker full-access default. They fail closed when the working directory or binary is unavailable. Reopening or restoring an existing session attaches without starting a second CLI. The separate **Launch worker** action remains the capacity-limited, tagged-worker mode.
