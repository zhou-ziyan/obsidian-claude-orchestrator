...
### Fixed
- **Intermittent Queue composer Enter**: Pressing Enter in an empty Queue composer now reliably sends the Queue head when one exists; with an empty Queue it falls back to a raw terminal Enter. Text + Enter still adds an item.
